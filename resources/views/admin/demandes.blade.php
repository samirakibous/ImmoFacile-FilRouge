                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        <tr>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <form method="POST" action="{{ route('demandes.update', $demande->id) }}" class="inline">
                                    @csrf
                                    @method('PUT')
                                    <button type="submit" class="bg-green-500 hover:bg-green-600 text-white py-1 px-3 rounded-md mr-2 transition duration-300">
                                        Accepter
                                    </button>
                                </form>
                                
                                <form method="POST" action="{{ route('demandes.destroy', $demande->id) }}" class="inline">
                                    @csrf
                                    @method('DELETE')
